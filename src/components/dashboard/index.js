...
function Dashboard(props) {
    ...
    useEffect(() => {
        const fetchData = debounce(async function () {
            ...
            last_month.ok.forEach((ok) => {
                barChart_lastmonth[ok[0] - 1].ok += ok[1];
                pieChart_lastmonth[0].value += ok[1];
            });
            last_month.fail.forEach((fail) => {
                barChart_lastmonth[fail[0] - 1].fail += fail[1];
                pieChart_lastmonth[1].value += fail[1];
            });
            ...
            for (
                let i = 1;
                i < last_month.statistic_predict_results.length;
                i++
            ) {
                if (i % 10 === 0) {
            ...
            current_month.ok.forEach((ok) => {
                barChart_currentmonth[ok[0] - 1].ok += ok[1];
                pieChart_currentmonth[0].value += ok[1];
            });
            current_month.fail.forEach((fail) => {
                barChart_currentmonth[fail[0] - 1].fail += fail[1];
                pieChart_currentmonth[1].value += fail[1];
            });
            ...
            for (
                let i = 1;
                i < current_month.statistic_predict_results.length;
                i++
            ) {
                if (i % 10 === 0) {
                    ...
                    value_tmp2 +=
                        current_month.statistic_predict_results[i - 1][1];
                }
            }
            setCurrentMonthData({
                pieChart: pieChart_currentmonth,
        ...
        <div id="dashboard">
            <Container className="lastest-report">
                ...
                <Row className="report-content">
                    <Col md="12" lg="6">
                        <h5 className="report-header pb-4 text-center">
                            Last month report
                        </h5>
                    </Col>
                    <Col md="12" lg="6">
